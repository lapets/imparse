...
(function (_, uxadt, richreports, imparse) {
  ...
  I.tokenizer = function(grammar) {
    ...
    var seqs = _.flatten(uxadt(cs).map(I.Choice(_, _, _), function(l, a, seq) { return seq; })); // Expressions
    var ts = uxadt(seqs).map(I.Terminal(_), function(t) { return t; }); // Terminals
    var rs = uxadt(seqs).map(I.RegExpr(_), function(r) { return r; }); // Regex
          ...
          return uxadt(ps).find(I.Production(_, _), function(ntP, choiceBlocks) {
            ...
            cr = choiceBlocks[cix]._(I.Choices(_), function(choices) {
              ...
              return uxadt(choices).find(I.Choice(_, _, _), function(name, assoc, sequence) {
                ...
                ._(I.May(_), function(innerSeq) {
                  var r = I.parse(g, tokens, innerSeq);
                  if (r != null) {
                    tokens = r[1]; 
                    if (r[0] === []) {
                      terminal_counter++;
                ...
                ._(I.Many(_), function(innerSeq) {
                  ...
                  } while (r != null && innerSeq.length > 0);
                  })
